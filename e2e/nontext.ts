 ...
 *
 * On this page the CONTROL-BOUNDARY half is the live one, and it caught two
 * real defects the first time it ran: `.btn-outline` ("Query a different
 * book") drew its only delineator as a `rgba(255,255,255,0.06)` border —
 * about 1.1:1 against the panel behind it in the dark theme — and
 * `.btn-danger` ("Simulate the servers colluding") had a translucent fill AND
 * a 40%-alpha border that both dissolved into the collusion block's red wash,
 * ~2:1 dark and worse light. Both were fixed rather than baselined. The other
 * controls each satisfy the rule a different way, which is why the oracle
 * judges outcomes rather than mechanisms: `.btn-primary` and the active
 * `.toggle-btn` are opaque teal fills; `.btn-secondary` documents in
 * `style.css` that its translucent fill CANNOT clear 3:1 on both themes and
 * carries an opaque `#12907e` border chosen for the overlap — this oracle is
 * what holds that comment to its numbers; the shared bar's `.cl-btn` border
 * mixes from `--cl-ink` (the fleet fix first landed in simon-period) because
 * mixing from a lab's `--accent` fails on dark accents at any percentage; and
 * the range slider is a native widget the author never repainted, which
 * 1.4.11 exempts and the `appearance` test below honours.
 *
 * The GENERATED-CONTENT half is inert here today: `style.css` declares not a
 * single `content` rule, and every glyph cue on the page (✓, ⚠, →, ⊕, ●, ·)
 * is a real text node that `contrast.ts` or its aria-hidden hand-measurements
 * cover. The half stays live at every driven state anyway, because the first
 * `::after` badge anyone adds would otherwise be invisible to every oracle in
 * this repo.
 *
 ...
 *    own identification and is not flagged (the lab's `.header-link`s and the
 *    footer's related-demo links are that case). Then either delineator will
 *    do, because either is enough for a sighted reader to find the edge:
 *      - the FILL against the surface just outside the element, or
 *      - the BORDER against that outside surface.
 *    An element passes if either clears 3:1. It fails only when the control
 *    genuinely dissolves into its panel — the `.btn-outline` case above.
 *
 *    A known boundary of the boundary: the book cards are clickable,
 *    Enter/Space-operable `role="listitem"` divs, which no selector below
 *    matches. They are identified primarily by their text (title, author,
 *    genre — all measured by `contrast.ts`), their selected state draws an
 *    opaque teal border plus ring, and their focus state has the global 3px
 *    teal `:focus-visible` outline; their idle 6%-alpha border is treated as
 *    decoration on a text-identified target, the same judgement the fleet
 *    applies to prose links.
 *
 * 2. GENERATED CONTENT, 4.5:1 (3:1 for large). `getComputedStyle(el, '::before')`
 *    reports the pseudo's `content`, `color`, `background` and font, so the
 *    ink is knowable; what is not knowable is its box, because a
 *    pseudo-element has no `getBoundingClientRect`. It is measured against its
 *    host's composited backdrop, which is right for a glyph painted inside its
 *    host; an absolutely-positioned pseudo that escapes is reported with
 *    `POSITIONED` in its detail so triage can see the measurement is against
 *    the wrong surface.
 *
 ...
 * background beneath — because a control's fill here is an `rgba()` over a
 * section that paints a `radial-gradient` wash, and reading the declared value
 * would measure something the page never paints.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
    ...
    /** Interpolate a gradient's stop list at fraction `t`, premultiplied. */
    ...
    /**
     * Evaluate one background-image layer at a document point. Same sampler as
     * `contrast.ts`: the gradients that matter here are the three sections'
     * faint `radial-gradient(ellipse at …)` washes, which sit directly behind
     * most of the controls this file judges — the surround a button must clear
     * 3:1 against includes that wash at the button's own location.
     */
    ...
    /**
     * The colour this element's own box paints at a point: its
     * background-color with every background-image layer sampled and
     * composited in paint order (first-listed layer on top).
     *
     * Reading `background-color` alone is not what a page paints — a section
     * whose surface is a wash-over-colour shorthand reports the colour but not
     * the wash, and a gradient-only box reports `rgba(0, 0, 0, 0)`. Elsewhere
     * in this fleet a `background-color`-only walk fell through to WHITE under
     * every control and judged the whole page against a surface that does not
     * exist — with the tell that the number came out IDENTICAL in both themes.
     */
    ...
    const CONTROL = [
      ...
      '[role=radio]',
      '[role=slider]',
      // A LINK STYLED AS A BUTTON is a UI component under 1.4.11: the shared
      // header's Menu and GitHub controls are `<a class="cl-btn">`, and with
      // no `a` in this list nothing would ever judge them. Restricted to
      // link-shaped BUTTONS — prose links identify themselves by their text
      // and are not 1.4.11 cases — and anything that slips through still has
      // to pass the "is it trying to draw itself as a control?" test below.
      // This lab's `.header-link`s and footer links match neither pattern and
      // are judged as prose links; the ink sitting on them is measured by the
      // 1.4.3 walk in `contrast.ts`.
      'a[role=button]',
      'a[class*=btn]',
      'a[class*=button]',
    ].join(',');
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // WCAG 1.4.11 exempts inactive components — here the shipped state of
      // `#query-btn`, disabled until a book is selected.
      ...
      // user agent and not modified by the author". A native range slider
      // still reports a background and a border to `getComputedStyle` — the
      // UA's own — so without this test the oracle judges the browser's widget
      // and reports a failure for a control the author never painted.
      // `appearance: auto` is exactly the signal that the author has NOT taken
      // the painting over (setting `accent-color`, as `.scaling-slider` does,
      // keeps the UA in charge); `appearance: none` says they have, and then
      // every pixel is theirs to answer for.
      ...
      for (const side of paintedSides) {
        ...
        // underneath the border. It matters here: `.btn-danger`'s translucent
        // red edge takes the button's own red-tinted fill as its backdrop, not
        // the collusion block outside it.
      ...
      if (hasOutline) {
        ...
        // surround — the opposite of the border above. This is the branch the
        // 3px teal `:focus-visible` outline lands in when a scan runs with a
        // control focused.
    ...
    // ── 2. generated content ────────────────────────────────────────────────
    // Inert on this page today — `style.css` declares no `content` rule — and
    // kept live at every driven state so the first pseudo-element glyph anyone
    // adds is gated rather than invisible to every oracle in the repo.
    ...
    for (const el of Array.from(document.querySelectorAll(rootSelector))) {
      ...
      for (const pseudo of ['::before', '::after']) {
        ...
        // is an image. `counter(...)`/`attr(...)` still render text and are
        // reported all the same, since the ink is what matters.
        ...
        if (rr < required) {
          // Is the pseudo positioned, and therefore likely escaping its host?
